...
**`critique` and `repeatability` fan out by default** (§auto-fanout), and the
brief carries **no `run` arg and no `--manual`**: either selects the serial hand
path, which no delegated run can walk — it parks at an owed critique second
pass or `stopped:repeatability-needs-fresh-session:run-<N+1>`. The router names
the bare lens, so relay it as handed. A harness that can't pin models per spawn
degrades itself: it emits the relaunch command and stamps `auto: unavailable
(harness) — manual relaunch`, which parks exactly as before.
...
`iter-N`/delta bracket when demoted, and that `critique`/`repeatability` fan
out: that span and that fan-out are where the cost lands.
...
- `critique`/`repeatability` were spawned bare — no `run` arg, no `--manual`; a
  park on either names a harness degradation or a real finding, never the hand
  path.
