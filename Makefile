...
#            -o $RDR_HOME/bin/rdr .
#
# A bare `go build -o bin/rdr .` leaves version="dev", which rdr-doctor 11b
...
BIN      := $(RDR_HOME)/bin/rdr

# The stamp doctor 11b compares against `git rev-parse --short HEAD`. It is
# the bare sha with no -dirty suffix, and deliberately: `rdr version` prints
...
## build: stamped binary at bin/rdr — the default, and what /rdr-init writes
build:
