...
wrote it and essentially never after — from Stage 7 on the RDR itself is the record,
so a Final RDR's evidence tree is a **write-only archive**. Never polish, amend,
re-format, or garden it post-lock; there is no reader to benefit. Terse at capture
is worth it, tidy after the fact is not.
