...
the **Determinacy trigger** fires — judged once against
[`3-repeatability.md` §Determinacy trigger](../prompts/pre-lock/3-repeatability.md#determinacy-trigger),
written as the `Determinacy:` line in Normative Contracts, and routed by
`--outcome repeatability`. Foundational already runs the full `repeatability` lens.
...
- **Exact state lives in a tool call, never in prose.** The *state* — counts,
  ...
  and the model never applies an operator to a state in its own text. Per-step accuracy on exact state
  ...
  This is why `§intrastate` is a dependency and not an accelerator, why
  `§rdr-resolve`, `§mechanical-gate`, and the `--tags`/`--json` facets exist,
  and why a sub-agent returns a verdict, not the list it derived it from.
