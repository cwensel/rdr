...
filename: `stages/` holds the front-half stage prompts (Seed → Reconcile →
...
[`../stages/`](../stages/) stage runs one of these (or, for the front half, its
`stages/NN-*.prompt.md`); the stage `.md` is the human operating manual, the prompt
file is the pasteable body the `/rdr-*` skill runs.

## Flow — the front-half stage prompts (`stages/`)

The pasteable bodies for the linear front-half stages, **extracted verbatim** from
each [`../stages/`](../stages/) stage doc's `Paste this` block so a skill (or a hand
driver) runs the prompt without re-parsing the doc. Each binds `{RDR_PATH}` /
`{RDR_RESOURCES}` / `{RDR_ENV}` / `{EVIDENCE_DIR}` from the workspace marker.

| Stage | File | Run by |
| --- | --- | --- |
| 1 Seed | [stages/01-seed.prompt.md](stages/01-seed.prompt.md) | `/rdr-seed` |
| 2 Propose | [stages/02-propose.prompt.md](stages/02-propose.prompt.md) | `/rdr-propose` |
| 3 Refine | [stages/03-refine.prompt.md](stages/03-refine.prompt.md) | `/rdr-refine` |
| 4 Resolve | [stages/04-resolve.prompt.md](stages/04-resolve.prompt.md) | `/rdr-resolve` |
...
| 6 Reconcile | [stages/06-reconcile.prompt.md](stages/06-reconcile.prompt.md) | `/rdr-reconcile` |
...
lenses below; Stages 7.1 and 8 dispatch into `gate/` and `implementation/`.
...
The **single-RDR** lenses are each one file, one paste block, reading only
`{RDR_PATH}` (grounding also reads the source the RDR cites). The `lens` outcome
of `models/rdr-status.toml` orders them per profile; numbering is file order.
...
- [**tooling-pass.md**](gate/tooling-pass.md) — the *mechanical pre-step of the
  ...
  8](../stages/07.0-finalize.md), just before the Gate's written responses, as a
...
- [**pairwise.md**](gate/pairwise.md) — the *cross-RDR* contradiction scan, run
  ...
  Cluster-reconcile](../stages/07.1-cluster-reconcile.md) (post-Final, per
...
In the [`../stages/`](../stages/README.md) recipe this prompt is **Stage 8
(Implement)**, the flow's terminus — [`stages/08-implement.md`](../stages/08-implement.md)
