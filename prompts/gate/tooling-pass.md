...
"$RDR_HOME/bin/rdr" lint --locking --records "$RDR_RECORDS" --repo <src-root> {RDR_NUMBER}
"$RDR_HOME/bin/rdr" inspect --json --records "$RDR_RECORDS" --repo <src-root> {RDR_NUMBER}
```
...
Corpus-wide: `rdr index --unresolved --records "$RDR_RECORDS" --repo <src-root>`.
