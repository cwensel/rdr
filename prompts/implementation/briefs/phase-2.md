...
`git commit` yourself — are `{RDR_HOME}/bin/rdr-leg-read <path> [--symbol S |
--range A-B]`, `rdr-leg-test --start {START_SHA} --since {START_EPOCH} [--full]
-- <test command>` and `rdr-leg-commit --start {START_SHA} --since {START_EPOCH}
--suite-green <true|false> -m "<subject>"`.
`next:`: `continue` → next item; `return-green` → REQ-MVV, then PASS;
