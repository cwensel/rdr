...
func missingRequired(d *scan.Document) []missing {
	...
	if c := d.ClassOf(); c != model.ClassRDR {
...
func resolutionFindings(d *scan.Document, terminal bool, opts Options) []Finding {
	...
	//
	// An Overrides field is prose, but the scanner reads its grammar:
	// only the record that LEADS a `;`-clause mints an overrides edge,
	// and the peers the clause names on the way are mentions. So the
	// pair is read off the typed edges of both records, and a peer not
	// in hand is not asserted.
	for _, e := range d.Edges {
